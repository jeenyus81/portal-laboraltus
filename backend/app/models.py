...
class Company(Base):
    ...
    user: Mapped["User | None"] = relationship(back_populates="company", uselist=False)
...
class Employee(Base):
    ...
    address: Mapped[str] = mapped_column(String(250), nullable=False)
    job_category: Mapped[str] = mapped_column(String(100), nullable=False)
    job_title: Mapped[str] = mapped_column(String(100), nullable=False)
    seniority_date: Mapped[date] = mapped_column(Date, nullable=False)
    social_security_number: Mapped[str] = mapped_column(String(30), unique=True, nullable=False)
    ...
    nominas: Mapped[list["Nomina"]] = relationship(back_populates="employee", cascade="all, delete-orphan")
...
class User(Base):
    ...
    company: Mapped["Company | None"] = relationship(back_populates="user")
...
class Nomina(Base):
    ...
    document_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    employee: Mapped["Employee"] = relationship(back_populates="nominas")
