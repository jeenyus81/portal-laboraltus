...
const API_URL = import.meta.env.VITE_API_URL || 'const API_URL = import.meta.env.VITE_API_URL || 'https://portal-laboraltus.onrender.com';'
...
function App() {
  ...
  const [hrAllActivities, setHrAllActivities] = useState([])
  const [hrAllActivitiesLoading, setHrAllActivitiesLoading] = useState(false)
  const [hrAllActivitiesError, setHrAllActivitiesError] = useState('')
  const [employeeAllActivities, setEmployeeAllActivities] = useState([])
  const [employeeAllActivitiesLoading, setEmployeeAllActivitiesLoading] = useState(false)
  const [employeeAllActivitiesError, setEmployeeAllActivitiesError] = useState('')
  ...
  async function loadHrActivityHistory() {
    setHrAllActivitiesLoading(true)
    setHrAllActivitiesError('')

    const token = localStorage.getItem('access_token')

    if (!token) {
      setHrAllActivities([])
      setHrAllActivitiesLoading(false)
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || 'No se pudo cargar el historial de actividad',
        )
      }

      setHrAllActivities(Array.isArray(data) ? data : [])
    } catch (err) {
      setHrAllActivities([])
      setHrAllActivitiesError(err.message)
    } finally {
      setHrAllActivitiesLoading(false)
    }
  }

  function handleOpenHrActivityHistory() {
    setHrAllActivitiesLoading(true)
    setHrAllActivitiesError('')
    setCompanyView('activity')
    setActiveMenu('dashboard')
    loadHrActivityHistory()
  }

  async function loadEmployeeActivityHistory() {
    setEmployeeAllActivitiesLoading(true)
    setEmployeeAllActivitiesError('')

    const token = localStorage.getItem('access_token')

    if (!token) {
      setEmployeeAllActivities([])
      setEmployeeAllActivitiesLoading(false)
      return
    }

    try {
      const response = await fetch(
        API_URL + '/api/activity/employee/all',
        {
          headers: {
            Authorization: 'Bearer ' + token,
          },
      ...
      setEmployeeAllActivities(Array.isArray(data) ? data : [])
    } catch (err) {
      setEmployeeAllActivities([])
      setEmployeeAllActivitiesError(err.message)
    } finally {
      setEmployeeAllActivitiesLoading(false)
    }
  }
  ...
  function handleOpenEmployeeActivityHistory() {
    setEmployeePortalView('activity')
    setEmployeeActiveMenu('dashboard')
    loadEmployeeActivityHistory()
  }
  ...
  async function handleEnterCompany(company) {
    ...
    const companyCredentials = storedCredentials[String(company.id)] || {
      username: '',
      password: '',
    }
    ...
    setCompanyCredentialsForm(companyCredentials)
    setCompanyCredentialsSaved(Boolean(companyCredentials.username && companyCredentials.password))
  ...
  async function handleSaveCompanyCredentials(event) {
    ...
    try {
      ...
      setCompanyCredentialsForm(savedCredentials)
      ...
      storedCredentials[String(selectedCompany.id)] = savedCredentials
      localStorage.setItem(
        COMPANY_CREDENTIALS_STORAGE_KEY,
  ...
  function handleOpenCompanyCredentials() {
    setCompanyCredentialsError('')
    setCompanyCredentialsMessage('')
  ...
  <div className="hr-dashboard">
    ...
    <div className="hr-dashboard-bottom">
      ...
      <section className="hr-dashboard-box">
        ...
        <button
          ...
          onClick={handleOpenHrActivityHistory}
        >
          Ver todo
        </button>
    ...
    <footer className="hr-dashboard-footer">
      © 2025 Portal Laboraltus. Todos los derechos reservados.
          ...
          {companyView === 'activity' && (
            <section className="company-page-section">
              <div className="company-page-header">
                <div>
                  <p className="eyebrow">HISTORIAL DE ACTIVIDAD</p>
              ...
              {hrAllActivitiesLoading ? (
                <div className="company-dashboard-message">Cargando actividad...</div>
              ) : hrAllActivitiesError ? (
                <div className="company-dashboard-message company-dashboard-error">{hrAllActivitiesError}</div>
              ) : hrAllActivities.length === 0 ? (
                <div className="company-empty-activity">
                  <strong>No hay actividad registrada</strong>
                  <p>Las acciones realizadas en el portal aparecerán aquí.</p>
                </div>
              ) : (
                <div className="company-activity-list company-activity-history-list">
                  {hrAllActivities.map((activity) => (
                    <button
                      type="button"
            ...
            <div className="contracts">
              ...
              <div className="profile hr-company-credentials-panel">
                <form onSubmit={handleSaveCompanyCredentials}>
                  ...
                  <div className="hr-company-credentials-actions hr-company-detail-actions">
    ...
    return (
      <main className="app employee-app">
        ...
        <div className="employee-main">
            ...
            <div className="employee-dashboard">
              ...
              <div className="employee-dashboard-bottom">
                <section className="employee-dashboard-box employee-dashboard-activity-box">
                  ...
                  <button
                    type="button"
                    className="employee-activity-view-all"
                    onClick={handleOpenEmployeeActivityHistory}
                  >
                    Ver todo
                  </button>
                </section>
          ...
          {employeePortalView === 'activity' && (
            <div className="employee-content-panel employee-activity-history-panel">
              <div className="employee-content-header">
                ...
                <div className="employee-contracts-landing-header-actions">
                  <button
                    type="button"
                    className="employee-back-button employee-activity-back-button"
                    onClick={() => {
                      setEmployeePortalView('dashboard')
                      setEmployeeActiveMenu('dashboard')
                    }}
              ...
              {employeeAllActivitiesLoading ? (
                <p className="muted">Cargando historial de actividad...</p>
              ) : employeeAllActivitiesError ? (
                <p className="error">{employeeAllActivitiesError}</p>
              ) : employeeAllActivities.length === 0 ? (
                <div className="empty-state">
                  <strong>No hay actividad registrada</strong>
                  <p>Las acciones relacionadas con tu perfil aparecerán aquí.</p>
                </div>
              ) : (
                <div className="employee-activity-history-list">
                  {employeeAllActivities.map((activity) => (
                    <div className="employee-activity-history-item" key={activity.id}>
                      <div className="hr-activity-icon">
                        {activity.icon || '•'}
                      </div>
                      <div className="hr-activity-content">
